...
internal entity
InitAsteroidA(v2 StartP, v2 EndP, real32 RotationRate)
{
    entity A = {};
    A.CenterP = StartP;
    A.Mass = 1.0f;
...
internal entity
InitAsteroidB(v2 StartP, v2 EndP, real32 RotationRate)
{
    entity A = {};
    A.CenterP = StartP;
    A.Mass = 1.0f;
...
extern "C" GAME_UPDATE_AND_RENDER(GameUpdateAndRender)
{
    ...
    // NOTE(Eric): Mouse position
    GameState->MousePos.Min = V2(Input->MouseX-5.0f, GameState->RenderHeight-Input->MouseY-5.0f);
    GameState->MousePos.Max = V2(Input->MouseX+5.0f, GameState->RenderHeight-Input->MouseY+5.0f);
    GameState->MousePos.Color = V3(0,1,1);
    GameState->MousePos.Fill = true;
    ...
    v2 ScreenOrigin = ScreenCenter;
    
    line *XAxis = GameState->DebugLines + 0;
    line *YAxis = GameState->DebugLines + 1;
    GameState->DebugLineCount = 2;
    
    XAxis->P1 = ScreenOrigin;
    XAxis->Color = V3(1,1,0);
    YAxis->P1 = ScreenOrigin;
    YAxis->Color = V3(0,1,1);
    
    XAxis->P2 = 100*V2(Cos(Angle), Sin(Angle));
    YAxis->P2 = V2(-XAxis->P2.y, XAxis->P2.x);
    
    for (real32 y = 0; y < 1.0f; y += 0.25f)
    {
