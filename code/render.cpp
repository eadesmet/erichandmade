...
{
    // NOTE(Eric): A good 50-60% of our rendering is in this routine,
    // so it's a good  candidate for optimization/SIMD if we want
    //BEGIN_TIMED_BLOCK(RenderSquare);
...
inline void
RenderWireBoundingBox(render_buffer *Render, v2 Min, v2 Max, v3 Color = {1,1,1}, b32 Fill = false, b32 IsGamePoints = false)
{
    ...
    if (IsGamePoints)
    {
        TopLeft = GamePointToScreenPoint(TopLeft);
        BottomLeft = GamePointToScreenPoint(BottomLeft);
        TopRight = GamePointToScreenPoint(TopRight);
        BottomRight = GamePointToScreenPoint(BottomRight);
    }
...
{
    RenderWireBoundingBox(Render, Box->Min, Box->Max, Box->Color, Box->Fill, Box->IsGamePoints);
}
...
//~ NOTE(Eric): Asteroids
inline void
RenderAsteroid(render_buffer* Render, entity* Asteroid)
{
    if (Asteroid->State != EntityState_InActive)
    {
        // NOTE(Eric): Rotate the points
        // So the points are already in relation to the center.
        // All I should need to do is find the angle between CenterP -> Point
        // Increment the angle by some amount, then find the new Point based on 
        // the new angle.
        for (u32 PointIndex = 0;
             PointIndex < Asteroid->PointCount;
             PointIndex++)
        {
            v2 *Point = Asteroid->Points + PointIndex;
            
            real32 PLength = Length(*Point);
            real32 PAngle = ATan2(Point->y, Point->x);
            PAngle += Asteroid->RotationRate;
            *Point = V2(Asteroid->CenterP.x + PLength*Cos(PAngle),
                        Asteroid->CenterP.y + PLength*Sin(PAngle))
                -Asteroid->CenterP;
        }
        
        // NOTE(Eric): Render the points
        for (u32 PointIndex = 0;
             PointIndex < Asteroid->PointCount;
             PointIndex++)
        {
            ...
            {
                v2 *Point1 = Asteroid->Points + PointIndex;
                v2 *Point2 = Asteroid->Points + PointIndex + 1;
                
                v2 P1 = GamePointToScreenPoint(Asteroid->CenterP + *Point1);
                v2 P2 = GamePointToScreenPoint(Asteroid->CenterP + *Point2);
                RenderLine(Render, P1, P2, 2, V3(.7,.5,.2));
            }
            else
            {
                v2 *Point1 = Asteroid->Points + PointIndex;
                v2 *Point2 = Asteroid->Points;
...
RenderAsteroidPositions(render_buffer* Render, entity* Asteroid)
{
    ...
    RenderSquare(Render, Screen_AsteroindCenterP, 2, V3(0,1,1));
    
    // NOTE(Eric): Bounding Box
    bounding_box BB = GetBoxFromPolygon(*Asteroid);
    RenderWireBoundingBox(Render, &BB);
    //AddDebugRenderBox(GameState, BB);
}
